...
We use the packages `here()` to point to different files *within our project*. (**note:** your file path will differ from this! Make sure you use the right path within YOUR project)

```{r}
### Read in data 
hp_data <- read_csv(here("data", "hp_aggression.csv"))
```
...
```{r}
ggplot(data = hp_clean, aes(x = mentions, y = aggressions)) +
...
```{r}
ggplot(data = hp_clean, aes(x = aggressions)) +
...
```{r}
ggplot(data = hp_clean, aes(x = book, y = aggressions)) +
